...
class BindingSiteChain: 
      ...
      auth_asym_ids   : List[str]= field(hash=True, compare=False)
...
def parse_polymer_nbhd(poly: PolymerRefs, structure: Structure):
    """A ligand-like polymer can figure in multiple instnaces in a structure.
    Multiple auth_asym_ids would then characterize individual instances(and corresponding chains in the structure)."""

    STATIC_ROOT = os.environ.get('STATIC_ROOT')
    for asym_id in poly.auth_asym_ids:
        outfile_json = os.path.join(
            STATIC_ROOT, poly.parent_rcsb_id.upper(), f'POLYMER_{asym_id}.json')

        residues: List[Residue] = get_polymer_residues(asym_id, structure)
        bs: BindingSite = get_poly_nbrs(residues, structure)
        bs.to_json(outfile_json)


def get_liglike_polymers(struct: str, profile:dict) -> List[PolymerRefs]:
...
def get_poly_nbrs(
    ...
    nbr_residues = list(filter(lambda res: res.parent_strand_id != parent_strand and res.residue_name in [*AMINO_ACIDS.keys(), *NUCLEOTIDES], nbr_residues))
    nbr_dict     = {}
...
if __name__ == "__main__":
    ...
    liglike_polys = get_liglike_polymers(PDBID, _structure_json_handle)
    ligands       = get_lig_ids(PDBID, _structure_json_handle)
