...
' | cypher-shell --format plain --database $DB_NAME;


# INTERPRO families
echo 'call apoc.load.json("file:///ontology/data/interpro.json") yield value
with value as v
merge (q:InterProFamily{ family_id:KEYS(v)[0],type:v[KEYS(v)[0]].type,description:v[KEYS(v)[0]].name})' | cypher-shell --format plain --database $DB_NAME;
...
do 
	...
	merge (inode)-[:interpro_go{annotation:go.interpro_class}]-(gonode)" | cypher-shell --format plain --database $DB_NAME;
done
...
do 
	...
	merge (inode)-[:interpro_pfam]-(pnode);" | cypher-shell --format plain --database $DB_NAME;
...
merge (pc:ProteinClass {class_id:key});" | cypher-shell --format plain --database $DB_NAME;
...
echo 'UNWIND [ 
  ...
  "mRNA"    ,
  "tRNA"    ]  as rnaclass
  merge (n:RNAClass {class_id:rnaclass})' | cypher-shell --format plain --database $DB_NAME;
